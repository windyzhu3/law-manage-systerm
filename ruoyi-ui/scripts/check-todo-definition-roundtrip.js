...
function run() {
  ...
  assert.ok(eventBuilder.includes('triggerEditor'), 'event builder must reserve editing for the Task14 trigger-rules resource')
  assert.ok(eventBuilder.includes('!triggerEditor') && eventBuilder.includes("$emit('input'"), 'definition editing must stay display-only unless the trigger dialog explicitly enables it')
