...
async function setup(page) {
  const state = { uploads: 0, complete: null, extension: null, summaries: 0, lists: 0 }; await page.context().addCookies([{ name: 'Admin-Token', value: 'x', url: 'http://127.0.0.1:4173/' }]); await page.addInitScript(() => { document.cookie = 'Admin-Token=x; path=/' })
  await page.route('**/prod-api/**', async route => { const path = new URL(route.request().url()).pathname.replace('/prod-api', ''); const req = route.request(); const ok = data => route.fulfill({ contentType: 'application/json', body: JSON.stringify({ code: 200, data }) })
    ...
    if (path === '/todo/8/form') return ok({ todoId: 8, businessType: 'LEAD', businessId: 8, ui: { config: { fields: [{ key: 'decision', label: 'Decision', required: true }] } }, dod: { config: { materials: [{ materialType: 'PROOF', label: 'Proof', minCount: 1 }] } }, extensionPolicy: { remainingRequestCount: 1, maxExtensionValue: 2, maxExtensionUnit: 'DAYS', proofRequired: true }, defaults: {}, materials: [] })
