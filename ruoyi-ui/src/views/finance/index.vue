<template>
  <div class="biz-page finance-page" :class="'biz-size-' + appSize">
    <biz-page-header eyebrow="FINANCE CENTER" :title="pageTitle" :description="pageDescription">
      ...
      <el-button v-if="mode === 'overview'" :size="controlSize" plain icon="el-icon-wallet" @click="switchMode('payment')">å¾…ç¡®è®¤å›žæ¬¾</el-button>
      ...
      <section class="finance-charts-grid">
        <article class="finance-card finance-chart-card">
          <div class="finance-card-title">
            <div>
              <h3>å›žæ¬¾è¶‹åŠ¿ï¼ˆä¸‡å…ƒï¼‰</h3>
              <p>è¿‘ 6 ä¸ªæœˆç¡®è®¤å›žæ¬¾é‡‘é¢ä¸Žå›žæ¬¾ç¬”æ•°</p>
            </div>
          ...
          <div v-if="receiveTrendRows.length" class="combo-chart">
            ...
            <div class="chart-legend"><span><i class="blue" />å›žæ¬¾é‡‘é¢</span><span><i class="green" />å›žæ¬¾ç¬”æ•°</span></div>
          </div>
          <el-empty v-else :image-size="72" description="æš‚æ— å›žæ¬¾è¶‹åŠ¿æ•°æ®" />
        </article>
        ...
        <article class="finance-card finance-chart-card">
          <div class="finance-card-title">
            <div>
              ...
              <p>å·²å¼€ç¥¨é‡‘é¢ä¸Žæ¡ˆä»¶è´¹ç”¨æ”¯å‡ºè¶‹åŠ¿</p>
            </div>
      ...
      <section class="finance-overview-grid">
        ...
        <article class="finance-card funnel-card">
          <div class="finance-card-title">
            <div>
              <h3>çº¿ç´¢è½¬åŒ–æ¼æ–—</h3>
              <p>ä»Žçº¿ç´¢é¢„è®¡é‡‘é¢åˆ°å®¢æˆ·ç­¾çº¦é‡‘é¢</p>
            </div>
        ...
        <article class="finance-card reminder-card">
          <div class="finance-card-title">
            <div>
              <h3>è´¢åŠ¡æé†’</h3>
              <p>ä¼˜å…ˆå¤„ç†å½±å“çŽ°é‡‘æµçš„äº‹é¡¹</p>
            </div>
          </div>
      ...
      <section class="finance-tables-grid">
        <biz-table-card :toolbar="false" :pagination="false">
          ...
          <el-table :data="pendingPayments" :size="controlSize">
            ...
            <el-table-column label="åº”æ”¶é‡‘é¢" width="120" align="right"><template slot-scope="{ row }">{{ formatMoney(row.receivableAmount) }}</template></el-table-column>
            <el-table-column label="è®¡åˆ’æ—¥æœŸ" prop="planReceiveDate" width="120" />
            <el-table-column label="æ“ä½œ" width="100" align="center" fixed="right"><template slot-scope="{ row }"><el-button v-hasPermi="['finance:payment:confirm']" :size="controlSize" type="text" @click="openPayment(row)">ç¡®è®¤</el-button></template></el-table-column>
          </el-table>
        </biz-table-card>
