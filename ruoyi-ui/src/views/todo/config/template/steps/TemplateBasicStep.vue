<template>
  <el-form ref="form" :model="model" :rules="rules" label-width="110px">
    ...
    <el-row :gutter="16">
      <el-col :span="8"><el-form-item label="业务类型" prop="businessType"><el-select v-model="model.businessType" :disabled="readonly || persisted" class="full"><el-option v-for="item in dict.type.law_todo_business_type" :key="item.value" :label="item.label" :value="item.value" /></el-select></el-form-item></el-col>
...
<script>
const BASIC_FIELDS = ['templateCode','templateName','businessType','businessStage','templateType','priority','changeSummary','impactScope','description']
const basicValue = value => BASIC_FIELDS.reduce((result, key) => { result[key] = value && value[key] !== undefined ? value[key] : ''; return result }, {})
export default {
  ...
  data() { return { syncing: false, model: basicValue(this.value), rules: { templateCode: [{ required: true, message: '请输入模板编码', trigger: 'blur' }], templateName: [{ required: true, message: '请输入模板名称', trigger: 'blur' }], businessType: [{ required: true, message: '请选择业务类型', trigger: 'change' }], businessStage: [{ required: true, message: '请选择业务阶段', trigger: 'change' }], templateType: [{ required: true, message: '请选择模板类型', trigger: 'change' }], priority: [{ required: true, message: '请选择优先级', trigger: 'change' }] } } },
  watch: { value: { deep: true, handler(value) { const next = basicValue(value); if (JSON.stringify(next) === JSON.stringify(this.model)) return; this.syncing = true; this.model = next; this.$nextTick(() => { this.syncing = false }) } }, model: { deep: true, handler(value) { if (this.syncing) return; this.$emit('input', basicValue(value)); this.$emit('dirty-change') } } },
