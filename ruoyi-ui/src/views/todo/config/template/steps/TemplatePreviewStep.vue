...
export default {
  ...
  props: { versionId: [Number, String], eventType: String, businessType: String, definitionReady: Boolean, sourceToken: String, definitionHash: String },
  ...
  methods: { formatDate(date) { const pad = value => String(value).padStart(2, '0'); return `${date.getFullYear()}-${pad(date.getMonth()+1)}-${pad(date.getDate())}T${pad(date.getHours())}:${pad(date.getMinutes())}:${pad(date.getSeconds())}` }, async loadCatalog() { const responses = await Promise.all([listTemplateHandlerCatalog(), listTemplateAutoActionCatalog()]); this.handlerCatalog = responses[0].data || []; this.autoActionCatalog = responses[1].data || [] }, async runSimulation() { if (!this.definitionReady || !this.definitionHash) { this.errorText = '当前草稿已变更，请先保存并完成发布预检'; return } this.errorText = ''; const tokenAtStart = this.sourceToken; let payload; try { payload = JSON.parse(this.payloadText); if (!payload || Array.isArray(payload) || typeof payload !== 'object') throw new Error() } catch (_) { this.errorText = '事件载荷必须是 JSON 对象'; return } this.running = true; try { const response = await simulateConfiguration({ requestId: `template-sim-${Date.now()}`, versionId: Number(this.versionId), eventType: this.eventType, businessType: this.businessType, businessId: Number(this.businessId), payload, effectiveAt: this.effectiveAt, taskCompletions: [], expectedDefinitionHash: this.definitionHash }); if (tokenAtStart !== this.sourceToken || !this.definitionReady) return; this.result = response.data || {} } catch (error) { this.errorText = (error && (error.msg || error.message)) || '模拟失败' } finally { this.running = false } }, validate() { return Promise.resolve(true) } }
}
</script>
