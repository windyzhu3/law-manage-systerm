...
export default {
  ...
  data() { return { syncing: false, model: { dodRuleIds: this.value.map(Number), composition: this.composition || 'ALL' }, rulesCatalog: [], validatorCatalog: [], nestedOpen: false, rules: { dodRuleIds: [{ type: 'array', required: true, message: '请选择至少一条 DoD 规则', trigger: 'change' }] } } },
  ...
  methods: { field(row, camel, snake) { return row && (row[camel] !== undefined ? row[camel] : row[snake]) }, ruleId(row) { return Number(this.field(row, 'id', 'id')) }, async loadRules() { const response = await listTemplateDodRuleCatalog(); this.rulesCatalog = response.data || [] }, async loadValidatorCatalog() { const response = await listTemplateValidatorCatalog(); this.validatorCatalog = response.data || [] }, move(index, delta) { const copy = this.model.dodRuleIds.slice(); const target = index + delta; copy.splice(target, 0, copy.splice(index, 1)[0]); this.model.dodRuleIds = copy }, openNested() { this.nestedOpen = true }, async nestedSaved(savedId) { this.nestedOpen = false; await this.loadRules(); const exact = Number(savedId); if (exact > 0 && this.rulesCatalog.some(item => this.ruleId(item) === exact) && !this.model.dodRuleIds.includes(exact)) this.model.dodRuleIds.push(exact) }, validate() { return new Promise(resolve => this.$refs.form.validate(valid => resolve(valid))) } }
}
</script>
