...
import { listDefinitions, listDefinitionVersions, copyDefinition, copyDefinitionVersion, publishDefinition, listTriggerRules, listWorkCalendars, listTodoDecisions } from '@/api/todo-definition'

export default {
  name: 'TodoConfig', components: { TemplateList, VersionDrawer, TriggerRuleTable, WorkCalendarTable, DecisionRegistry },
  data() { return { tab: 'templates', loading: false, templates: [], triggers: [], calendars: [], decisions: [], versionRows: [], drawer: false, current: null } },
  created() { this.load() },
  methods: {
    load() {
      this.loading = true
      return Promise.all([listDefinitions(), listTriggerRules(), listWorkCalendars(), listTodoDecisions()])
        .then(([templates, triggers, calendars, decisions]) => { this.templates = templates.data || []; this.triggers = triggers.data || []; this.calendars = calendars.data || []; this.decisions = decisions.data || [] })
