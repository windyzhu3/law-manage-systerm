    ...
    <el-form ref="form" :model="form" :rules="rules" label-width="120px" class="rule-drawer-form">
      <el-collapse v-model="sections">
        <el-collapse-item title="基本信息" name="basic">
          ...
          <el-row :gutter="16"><el-col :span="12"><el-form-item label="SLA 类型" prop="slaType"><el-select v-model="form.slaType" :disabled="readonly || copyMode" class="full-width"><el-option v-for="item in dict.type.law_todo_sla_type" :key="item.value" :label="item.label" :value="item.value" /></el-select></el-form-item></el-col><el-col :span="12"><el-form-item label="状态" prop="status"><el-select v-model="form.status" :disabled="readonly || copyMode" class="full-width"><el-option v-for="item in dict.type.law_todo_rule_status" :key="item.value" :label="item.label" :value="item.value" /></el-select></el-form-item></el-col></el-row>
        </el-collapse-item>
...
const emptyForm = () => ({ slaRuleId: null, ruleCode: '', ruleName: '', slaType: '', durationValue: 60, durationUnit: '', calendarCode: '', startStrategy: '', softRemindPercent: 80, hardRemindPercent: 100, escalatePercent: 150, pausePolicyJson: '', escalationPolicyJson: '', autoActionJson: '', timeoutStrategy: '', status: '0', version: 0, referenceCount: 0, updateTime: '', updateBy: '' })
...
export default {
  ...
  methods: {
    ...
    finishSaved() { this.$emit('saved'); this.$refs.drawer.closeAfterSave() },
...
.full-width { width: 100%; }.validation-tip { margin: -8px 0 0 120px; color: #dc2626; font-size: 12px; }.drawer-note { margin: 8px 0; color: #64748b; font-size: 12px; }::v-deep .el-collapse-item__content { padding-bottom: 12px; }
</style>
