<template>
  <config-page-shell title="发布记录" subtitle="基于不可变模板版本与发布动作账本聚合，历史版本始终只读。" :loading="loading" :empty="!loading&&!rows.length" empty-text="暂无发布记录">
    ...
    <template #metrics>
      ...
      <config-metric-card label="当前发布" :value="publishedCount" icon="el-icon-circle-check" tone="success" />
      <config-metric-card label="已退役" :value="retiredCount" icon="el-icon-remove-outline" tone="warning" />
      <config-metric-card label="回滚来源" :value="rollbackCount" icon="el-icon-refresh-left" />
    </template>
    <template #filters>
      <el-card shadow="never" class="filter-card"><el-form :inline="true" size="small">
        ...
        <el-form-item><el-select v-model="query.status" clearable placeholder="发布状态"><el-option label="已发布" value="PUBLISHED" /><el-option label="已退役" value="RETIRED" /></el-select></el-form-item>
    ...
    <el-card shadow="never">
      <el-table :data="rows" @row-click="openDetail">
        ...
        <el-table-column prop="status" label="发布状态" width="100"><template slot-scope="{row}"><el-tag size="small" :type="row.status==='PUBLISHED'?'success':'info'">{{ row.status }}</el-tag></template></el-table-column>
