<template>
  <div class="definition-builder">
    <el-form-item label="负责人规则" prop="owner.config.type">
      ...
      <el-select v-else-if="model.type === 'ROLE'" :value="model.operand" :disabled="readonly" filterable placeholder="选择角色" @input="change('operand', $event)">
        <el-option v-for="role in roles" :key="role.roleId" :label="`${role.roleName}（${role.roleKey}）`" :value="role.roleId" />
      </el-select>
      <el-select v-else-if="model.type === 'DEPT'" :value="model.operand" :disabled="readonly" filterable placeholder="选择部门" @input="change('operand', $event)">
...
export default {
  ...
  data() { return { roles: [], departments: [] } }, computed: { model() { return { type: 'PAYLOAD', operand: 'ownerId', ...this.value } } },
  ...
  methods: { change(key, value) { this.$emit('input', { ...this.model, [key]: value }) } }
}
</script>
