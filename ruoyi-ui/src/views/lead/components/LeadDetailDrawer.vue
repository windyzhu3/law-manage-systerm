  ...
  <el-drawer :visible.sync="visible" size="920px" :custom-class="'lead-detail-drawer ' + sizeClass" append-to-body :with-header="false" @close="$emit('close')">
    <div v-if="lead" class="detail-shell">
      <main>
        ...
        <div class="detail-actions"><el-button v-if="canFirstContact" v-hasPermi="['lead:first-contact:handle']" type="primary" icon="el-icon-phone-outline" @click="$emit('first-contact',lead)">处理首联</el-button><el-button v-if="canEdit" v-hasPermi="['lead:edit']" icon="el-icon-edit" @click="$emit('edit',lead)">编辑</el-button><el-button v-if="canAssign" v-hasPermi="['lead:assign']" icon="el-icon-user" @click="$emit('assign',lead)">分配</el-button><el-button v-if="canFollow" v-hasPermi="followPerms" icon="el-icon-plus" @click="$emit('follow',lead)">新增跟进</el-button></div>
        ...
        <section><h3>流程状态</h3><dl><dt>标签确认</dt><dd>{{ lead.tagConfirmStatus || '-' }}</dd><dt>首联状态</dt><dd>{{ lead.firstContactStatus || '-' }}</dd><dt>首联结果</dt><dd>{{ lead.firstContactResult || '-' }}</dd><dt>重试窗口</dt><dd>{{ lead.retryStage || '-' }}</dd><dt>处置位置</dt><dd>{{ lead.disposition || 'ACTIVE' }}</dd><dt>下次重试</dt><dd>{{ lead.nextRetryTime || '-' }}</dd></dl></section>
        <div class="todo-sla-caption"><strong>Todo / SLA</strong><span>当前待办、负责人和截止时间以流程引擎为准</span></div>
      ...
      <aside>
        ...
        <el-tabs class="operational-tabs">
          <el-tab-pane label="通话证据"><lead-call-timeline :lead-id="lead.leadId" /></el-tab-pane>
          <el-tab-pane label="重试计划"><lead-retry-timeline :lead-id="lead.leadId" /></el-tab-pane>
        </el-tabs>
...
import LeadRetryTimeline from './LeadRetryTimeline'
import { listBusinessTodos } from '@/api/todo'
import { allowed } from '../lead-todo-ui'
export default {
  name:'LeadDetailDrawer',
  components: { BusinessTodoSummary, LeadCallTimeline, LeadRetryTimeline },
  ...
  data(){return{firstContactAllowed:false,actionGeneration:0}},
  computed:{visible:{get(){return this.value},set(value){this.$emit('input',value)}},initial(){return(this.lead.contactName||this.lead.leadName||'线').slice(0,1)},amount(){return this.lead.estimatedAmount ? '¥ ' + Number(this.lead.estimatedAmount).toLocaleString() : '-'},terminal(){return ['3','4','5'].includes(this.lead.status)},inPool(){return this.lead.poolStatus === '1' || this.lead.disposition === 'PUBLIC_POOL'},inDeadPool(){return this.lead.disposition === 'DEAD_POOL'},canEdit(){return !this.terminal && !this.inDeadPool},canAssign(){return !this.terminal && !this.inDeadPool},canFollow(){return !this.terminal && !this.inPool && !this.inDeadPool && !!this.lead.ownerId},canFirstContact(){return this.canFollow && this.lead.tagConfirmStatus === 'CONFIRMED' && this.lead.firstContactStatus !== 'COMPLETED' && this.firstContactAllowed}},
  watch:{value(open){if(open)this.loadFirstContactAction()},'lead.leadId'(){if(this.value)this.loadFirstContactAction()}},
  methods:{
    maskMobile(value){return value ? value.replace(/(\d{3})\d{4}(\d{4})/,'$1****$2') : '未填写电话'},
    followType(type){const item=this.followTypeOptions.find(option=>String(option.value)===String(type));return item?item.label:(type||'-')},
    followIcon(type){return({phone:'phone',wechat:'wechat',meeting:'peoples',email:'email'})[type]||'message'},
    loadFirstContactAction(){
      this.firstContactAllowed=false
      if(!(Number(this.lead&&this.lead.leadId)>0))return
      const generation=++this.actionGeneration
      listBusinessTodos('LEAD',this.lead.leadId,{pageNum:1,pageSize:100}).then(response=>{
        if(generation!==this.actionGeneration)return
        const todo=(response.rows||[]).find(row=>(row.templateCode||row.template_code)==='TD-001'&&!['COMPLETED','CANCELLED'].includes(row.status||row.todoStatus))
        this.firstContactAllowed=!!todo&&allowed(todo,'complete')
      }).catch(()=>{if(generation===this.actionGeneration)this.firstContactAllowed=false})
    }
  }
}
</script>
...
.todo-sla-caption{display:flex;align-items:center;justify-content:space-between;margin-top:16px;color:#0f172a}.todo-sla-caption span{color:#64748b;font-size:11px}.operational-tabs{margin-top:22px;padding-top:10px;border-top:1px solid #e2e8f0}.operational-tabs .call-timeline header,.operational-tabs .retry-timeline header{display:block}.operational-tabs .call-timeline header .el-button{margin-top:8px}
