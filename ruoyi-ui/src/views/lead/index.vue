<template>
  <div class="lead-page" :class="'lead-size-' + appSize">
          ...
          <div class="table-card">
            ...
            <div class="lead-table-wrap">
              <el-table v-loading="loading" :data="leadList" size="mini" @selection-change="selection => ids = selection.map(item => item.leadId)">
                ...
                <el-table-column label="客户信息" min-width="150" align="center"><template slot-scope="{row}"><a class="lead-link" @click="openDetail(row)">{{ row.contactName || row.leadName }}</a><span class="sub-text">{{ maskMobile(row.mobile) }}</span></template></el-table-column>
                ...
                <el-table-column label="操作" :width="operationColumnWidth" align="center" class-name="small-padding fixed-width lead-operation-column" fixed="right"><template slot-scope="{row}">
                  ...
                  <template v-else-if="mode === 'pool'"><el-button v-if="canClaim(row)" v-hasPermi="['lead:pool:claim']" size="mini" type="text" icon="el-icon-user" @click="claim(row)">领取</el-button><el-button v-hasPermi="['lead:query']" size="mini" type="text" icon="el-icon-view" @click="openDetail(row)">详情</el-button></template>
                  <template v-else><span class="action-buttons"><el-button v-hasPermi="['lead:query']" size="mini" type="text" icon="el-icon-view" @click="openDetail(row)">查看</el-button><el-button v-if="canEdit(row)" v-hasPermi="['lead:edit']" size="mini" type="text" icon="el-icon-edit" @click="openLeadDialog(row)">编辑</el-button><el-button v-if="canFollow(row)" v-hasPermi="['lead:followup:add']" size="mini" type="text" icon="el-icon-chat-line-round" @click="openFollowDialog(row)">跟进</el-button><el-button v-if="canAssign(row)" v-hasPermi="['lead:assign']" size="mini" type="text" icon="el-icon-user" @click="openAssignDialog(row)">分配</el-button><el-button v-if="canMovePool(row)" v-hasPermi="['lead:pool:move']" size="mini" type="text" icon="el-icon-office-building" @click="toPool(row)">公海</el-button><el-button v-if="canConvert(row)" v-hasPermi="['lead:convert']" size="mini" type="text" icon="el-icon-circle-check" @click="convert(row)">转化</el-button><el-button v-hasPermi="['lead:remove']" size="mini" type="text" icon="el-icon-delete" class="danger-text" @click="remove(row)">删除</el-button></span></template>
    ...
    <lead-detail-drawer v-model="detailDrawer" :lead="detail" :followups="detailFollowups" :source-name="settingName('source',detail && detail.sourceCode)" :status="statusMeta(detail && detail.status)" :priority-name="dictLabel('law_lead_priority', detail && detail.priority)" :pool-status-name="dictLabel('law_lead_pool_status', detail && detail.poolStatus)" :follow-type-options="dict.type.law_lead_follow_type" :size-class="'lead-size-' + appSize" @refresh="loadDetailFollowups" @edit="openLeadDialog" @assign="openAssignDialog" @follow="openFollowDialog" />
  </div>
</template>
