...
public class TodoSlaController extends BaseController
{
    ...
    @PreAuthorize("@ss.hasPermi('todo:submit')") @PostMapping("/{id}/pause") public AjaxResult pause(@PathVariable Long id){return service.pause(id,SecurityUtils.getUserId(),LocalDateTime.now())?success():error("SLA当前不能暂停");}
    @PreAuthorize("@ss.hasPermi('todo:submit')") @PostMapping("/{id}/resume") public AjaxResult resume(@PathVariable Long id){return service.resume(id,SecurityUtils.getUserId(),LocalDateTime.now())?success():error("SLA当前不能恢复");}
}
