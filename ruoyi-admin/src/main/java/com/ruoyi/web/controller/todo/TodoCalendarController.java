...
public class TodoCalendarController extends BaseController
{
    ...
    @PreAuthorize("@ss.hasPermi('todo:calendar:manage')") @PostMapping public AjaxResult save(@Valid @RequestBody CalendarCommand value){return toAjax(service.save(value,actor()));}
    @PreAuthorize("@ss.hasPermi('todo:calendar:manage')") @PutMapping public AjaxResult update(@Valid @RequestBody CalendarCommand value){return toAjax(service.save(value,actor()));}
