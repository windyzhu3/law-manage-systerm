...
public class TodoTemplateController
{
    ...
    @PreAuthorize("@ss.hasPermi('todo:template:manage')") @PostMapping("/{id}/publish") public AjaxResult publish(@PathVariable Long id,@Valid @RequestBody PublishCommand c){return AjaxResult.success(service.publish(id,c.versionNo(),c.ownerRuleJson(),c.dodRuleJson(),c.slaRuleJson(),c.nextRuleJson(),SecurityUtils.getUsername()));}
}
