...
public class TodoTemplateController
{
    ...
    @PreAuthorize("@ss.hasPermi('todo:definition:edit')") @PutMapping("/version/{versionId}") public AjaxResult updateDraft(@PathVariable Long versionId,@Valid @RequestBody UpdateDraftCommand c){if(!versionId.equals(c.versionId()))return AjaxResult.error("版本ID不一致");return AjaxResult.success(definitions.updateDraft(c,actor()));}
