...
{
    private final TodoQueryService query;private final TodoCommandService command;
    public TodoController(TodoQueryService query,TodoCommandService command){this.query=query;this.command=command;}
    ...
    @PreAuthorize("@ss.hasPermi('todo:query')") @GetMapping("/{id}") public AjaxResult detail(@PathVariable Long id){return success(query.detail(id,SecurityUtils.getUserId(),SecurityUtils.getDeptId()));}
