...
class PhaseTwoDatabaseInvariantTest
{
    ...
    {
        String url=MigrationTestDatabase.migrate();try(Connection c=DriverManager.getConnection(url,MigrationTestDatabase.user(),MigrationTestDatabase.password())){assertEquals(14,count(c,"select count(*) from todo_template where template_code in ('CONTRACT_REVIEW','CONTRACT_SIGN','PAYMENT_CONFIRM','INVOICE_HANDLE','CASE_CREATE_CHECK','CASE_ASSIGN','CASE_ACCEPT','CASE_REASSIGN','CASE_TRANSFER_REVIEW','MATTER_NODE_HANDLE','MATTER_EXPENSE_REVIEW','MATTER_DOCUMENT_SUPPLY','CASE_CLOSE_CONFIRM','CASE_ARCHIVE_CONFIRM')"));assertEquals(20,count(c,"select count(*) from todo_template t join todo_template_version v on v.template_id=t.template_id where t.template_code<>'LEAD_FIRST_CONTACT' and v.status='PUBLISHED'"));assertEquals(16,count(c,"select count(*) from todo_trigger_rule r join todo_template t on t.template_id=r.template_id where t.template_code<>'LEAD_FIRST_CONTACT' and r.enabled='Y'"));assertEquals(0,count(c,"select count(*) from todo_template_version where status='PUBLISHED' and owner_rule_json is null"));assertEquals(10,count(c,"select count(distinct perms) from sys_menu where perms in ('todo:chain:query','todo:operations:list','todo:force:complete','todo:force:cancel','todo:batch:transfer','todo:sla:waive','todo:regenerate','todo:definition:list','todo:definition:edit','todo:definition:publish')"));
