...
public class CaseWorkflowSupport
{
    ...
    public void statusLog(Long id,String from,String to,String action,String content,BusinessActor actor){requireDict("law_case_status_action",action,"案件状态动作不合法");Map<String,Object> m=new HashMap<>();m.put("caseId",id);m.put("fromStatus",from);m.put("toStatus",to);m.put("actionType",action);m.put("content",content);m.put("createBy",actor.userName());if(mapper.insertStatusLog(m)<=0)throw new ServiceException("案件状态记录创建失败");}
    ...
    public void notice(String title,String content,BusinessActor actor){SysNotice n=new SysNotice();n.setNoticeTitle(title);n.setNoticeType("1");n.setNoticeContent(content);n.setStatus("0");n.setCreateBy(actor.userName());n.setRemark("案管中心");notices.insertNotice(n);}
