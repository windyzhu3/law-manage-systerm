...
public class CaseTransferService
{
    ...
    public int approve(Map<String, Object> command)
    {
        ...
        Long caseId = id(transfer.get("case_id"), "请选择案件"); queryService.caseDetail(caseId);
        ...
        command.put("approverId", SecurityUtils.getUserId()); command.put("approverName", SecurityUtils.getLoginUser().getUser().getNickName()); command.put("updateBy", SecurityUtils.getUsername());
        int rows = mapper.updateTransferApproval(command); changed(rows, "转案状态已变化，请刷新后重试");
        Map<String, Object> update = statusUpdate(caseId, CaseStatus.TRANSFERRING.code(), CaseStatus.PROCESSING.code(), "案件办理中");
        if (CaseTransferStatus.PASSED.code().equals(action)) { update.put("mainLawyerId", transfer.get("to_lawyer_id")); update.put("mainLawyerName", transfer.get("to_lawyer_name")); }
        changed(mapper.updateCaseStatus(update), "案件状态已变化，请刷新后重试");
        support.statusLog(caseId, CaseStatus.TRANSFERRING.code(), CaseStatus.PROCESSING.code(), "transfer_approve", "转案审批：" + action + "，" + opinion);
        support.notice("转案审批已处理", "转案单 " + transfer.get("transfer_no") + " 审批结果：" + label(action) + "，案件已回到办理流程。");
        publish(BusinessEventType.CASE_TRANSFER_APPROVED, caseId, text(transfer.get("case_no")), data("transferId", transferId, "action", action));
        return rows;
    }
