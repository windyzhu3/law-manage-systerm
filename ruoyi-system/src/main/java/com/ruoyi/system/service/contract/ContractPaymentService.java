...
public class ContractPaymentService
{
    ...
    @Transactional
    public int confirm(Long planId,String receivedAmount,String remark,String paymentMethod,String voucherUrl,BusinessActor actor)
    {
        Map<String,Object> plan=planRaw(planId); requireCollectable(plan);
        ...
        Map<String,Object> update=baseUpdate(planId,plan,CONFIRMED,actor.userName());
        ...
        if(voucherUrl!=null&&!voucherUrl.isBlank()){Map<String,Object> file=new HashMap<>();file.put("contractId",contractId(plan));file.put("attachmentType","PAYMENT_PROOF");file.put("fileName","payment-"+planId);file.put("fileUrl",voucherUrl);file.put("createBy",actor.userName());changed(mapper.insertAttachment(file));}
        log(plan,current,CONFIRMED,"fee_confirm",content,actor.userName());
