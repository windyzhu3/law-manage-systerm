...
public class ContractLifecycleService
{
    ...
    public int submit(Long contractId)
    {
        ...
        if (!DRAFT.equals(contract.getContractStatus())) throw new ServiceException("只有草稿状态合同可以提交审批");
    ...
    {
        if (StringUtils.isEmpty(opinion)) throw new ServiceException("审批意见必填");
        ...
        BizContract contract = queryService.contract(contractId);
        if (!REVIEWING.equals(contract.getAuditStatus())) throw new ServiceException("只有审核中的合同可以审批");
    ...
    {
        BizContract contract = queryService.contract(contractId);
        if (!PASSED.equals(contract.getAuditStatus())) throw new ServiceException("只有审批通过的合同可以签署");
        ...
        if (SIGNED.equals(contract.getSignStatus())) throw new ServiceException("合同已签订，不能重复签署");
