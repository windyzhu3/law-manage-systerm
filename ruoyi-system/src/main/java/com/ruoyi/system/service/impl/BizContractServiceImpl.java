...
public class BizContractServiceImpl implements IBizContractService
{
    ...
    public int submitContract(Long contractId)
    {
        ...
        {
            throw new ServiceException("å½“å‰å®¡æ ¸çŠ¶æ€ä¸å…è®¸æäº¤");
        }
        if (!CONTRACT_DRAFT.equals(contract.getContractStatus()))
        {
            throw new ServiceException("åªæœ‰è‰ç¨¿çŠ¶æ€åˆåŒå¯ä»¥æäº¤å®¡æ‰¹");
        }
        ...
        insertStatusLog(contractId, contract.getAuditStatus(), AUDIT_REVIEWING, "submit", "æäº¤å®¡æ‰¹");
    ...
    public int approveContract(Long contractId, String action, String opinion)
    {
        ...
        {
            throw new ServiceException("å®¡æ‰¹æ„è§å¿…å¡«");
        }
        assertDictValue("law_contract_approval_action", action, "å®¡æ‰¹åŠ¨ä½œä¸åˆæ³•");
        if (!"pass".equals(action) && !"reject".equals(action) && !"back".equals(action))
        {
            throw new ServiceException("å®¡æ‰¹åŠ¨ä½œä¸åˆæ³•");
        }
        BizContract contract = selectContractById(contractId);
        if (!AUDIT_REVIEWING.equals(contract.getAuditStatus()))
        {
            throw new ServiceException("åªæœ‰å®¡æ ¸ä¸­çš„åˆåŒå¯ä»¥å®¡æ‰¹");
        }
    ...
    public int signContract(Long contractId, String signStatus)
    {
        ...
        {
            throw new ServiceException("åªæœ‰å®¡æ‰¹é€šè¿‡çš„åˆåŒå¯ä»¥ç­¾ç½²");
        }
        ...
        {
            throw new ServiceException("å½’æ¡£ã€ä½œåºŸæˆ–ç»ˆæ­¢çš„åˆåŒä¸å…è®¸ç­¾ç½²");
        }
        assertDictValue("law_contract_sign_status", signStatus, "ç­¾ç½²çŠ¶æ€ä¸åˆæ³•");
        if (!SIGN_SIGNED.equals(signStatus) && !SIGN_PARTIAL.equals(signStatus))
        {
            throw new ServiceException("ç­¾ç½²çŠ¶æ€ä¸åˆæ³•");
        }
        if (SIGN_SIGNED.equals(contract.getSignStatus()))
        {
            throw new ServiceException("åˆåŒå·²ç­¾è®¢ï¼Œä¸èƒ½é‡å¤ç­¾ç½²");
        }
        String expectedContractStatus = CONTRACT_DRAFT;
        String content = SIGN_PARTIAL.equals(signStatus) ? "åˆåŒéƒ¨åˆ†ç­¾ç½²" : "åˆåŒç­¾ç½²";
        if (SIGN_PARTIAL.equals(contract.getSignStatus()))
        {
            ...
            {
                throw new ServiceException("éƒ¨åˆ†ç­¾è®¢çš„åˆåŒåªèƒ½è¡¥é½ä¸ºå·²ç­¾è®¢");
            }
            expectedContractStatus = CONTRACT_PERFORMING;
            content = "åˆåŒè¡¥é½ç­¾ç½²";
        }
        else if (!CONTRACT_DRAFT.equals(contract.getContractStatus()))
        {
            throw new ServiceException("åªæœ‰è‰ç¨¿æˆ–éƒ¨åˆ†ç­¾è®¢çš„å±¥çº¦ä¸­åˆåŒå¯ä»¥ç­¾ç½²");
        }
    ...
    {
        String archiveReason = requiredReason(reason, "å½’æ¡£è¯´æ˜å¿…å¡«");
        BizContract contract = selectContractById(contractId);
        if (!CONTRACT_PERFORMING.equals(contract.getContractStatus()))
        {
            throw new ServiceException("åªæœ‰å±¥çº¦ä¸­çš„åˆåŒå¯ä»¥å½’æ¡£");
        }
