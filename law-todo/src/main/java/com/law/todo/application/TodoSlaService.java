...
public class TodoSlaService
{
    ...
    @Transactional public int scanAndEscalate(LocalDateTime now)
    {
        ...
        {
            Long id=Long.valueOf(String.valueOf(value(item,"todo_id","todoId")));int version=Integer.parseInt(String.valueOf(value(item,"version","version")));
            ...
            int fired=markIfDue(id,"REMINDED_80",p80,version,now);changed+=fired;version+=fired;
            fired=markIfDue(id,"OVERDUE_100",p100,version,now);changed+=fired;version+=fired;
            fired=markIfDue(id,"ESCALATED_150",p150,version,now);changed+=fired;
        }
        return changed;
    }
    ...
    public ThresholdPlan planThresholds(LocalDateTime start,LocalDateTime due,WorkCalendar calendar)
    {
        ...
        return new ThresholdPlan(calculator.addWorkingMinutes(start,total*80/100,calendar),calculator.addWorkingMinutes(start,total,calendar),calculator.addWorkingMinutes(start,total*150/100,calendar));
    }
    ...
    private int markIfDue(Long id,String threshold,LocalDateTime planned,Integer version,LocalDateTime now){if(planned==null||planned.isAfter(now))return 0;int changed=mapper.markSlaThreshold(id,threshold,planned,version,now);if(changed>0){mapper.insertSlaNotification(id,threshold,now);if("ESCALATED_150".equals(threshold))mapper.insertSupervisorEscalationNotification(id,now);}return changed;}
