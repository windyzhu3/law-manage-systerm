...
public class TodoAutoActionConfiguration
{
    ...
    @Bean TodoAutoActionCapability transferCapability(TodoCommandService commands){return capability("TRANSFER",new TodoAutoActionCapability.Descriptor("TRANSFER",List.of("DUE","SLA_80","SLA_100","SLA_150"),TodoAutoActionCapability.Descriptor.commonRetryFields(),List.of(new TodoAutoActionCapability.Field("targetOwnerId","number",true,1,null,"Target owner","TODO_AUTO_ACTION_TRANSFER_OWNER_INVALID"))),(todo,rule,actor)->{commands.autoTransfer(todo.getTodoId(),command(todo.getTodoId(),rule),actor);return AutoActionResult.success();});}
    ...
    private TodoAutoActionCapability capability(String type,Executor executor){return capability(type,new TodoAutoActionCapability.Descriptor(type,List.of("DUE","SLA_80","SLA_100","SLA_150"),TodoAutoActionCapability.Descriptor.commonRetryFields(),List.of()),executor);}
    private TodoAutoActionCapability capability(String type,TodoAutoActionCapability.Descriptor descriptor,Executor executor){return new TodoAutoActionCapability(){public String actionType(){return type;}public Descriptor descriptor(){return descriptor;}public AutoActionResult execute(TodoInstance todo,AutoActionRule rule,Actor actor){return executor.execute(todo,rule,actor);}};}
