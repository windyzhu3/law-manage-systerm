...
public class TodoCommandService
{
    ...
    @Transactional public TodoInstance claim(Long id,ActionCommand c,Actor a){if(repeated(c))return mapper.selectById(id);TodoInstance t=require(id);if(!access.canClaim(t,a.userId(),a.deptId()))deny();return transition(t,TodoStatus.CLAIMED,a.userId(),"CLAIM",c,a);}
    ...
    @Transactional public TodoInstance complete(Long id,ActionCommand c,Actor a){if(repeated(c))return mapper.selectById(id);TodoInstance t=requireOwner(id,a);validateDod(t,c);TodoInstance completed=transition(t,TodoStatus.COMPLETED,null,"COMPLETE",c,a);for(TodoCompletionHandler h:completionHandlers)if(h.supports(completed))h.complete(completed,c.payload(),a.userId(),a.userName());createNext(completed);return completed;}
    @Transactional public TodoInstance returnTodo(Long id,ActionCommand c,Actor a){if(repeated(c))return mapper.selectById(id);TodoInstance t=requireVisible(id,a);return transition(t,TodoStatus.RETURNED,null,"RETURN",c,a);}
    @Transactional public TodoInstance transfer(Long id,ActionCommand c,Actor a){if(repeated(c))return mapper.selectById(id);TodoInstance t=requireOwner(id,a);TodoStatus current=TodoStatus.fromCode(t.getStatus());if(current.isTerminal())throw new TodoException("TODO_TERMINAL","终态待办不能转派");Long target=Long.valueOf(String.valueOf(c.payload().get("targetOwnerId")));return transition(t,current,target,"TRANSFER",c,a);}
    ...
    private TodoInstance requireVisible(Long id,Actor a){TodoInstance t=require(id);if(!access.canView(t,a.userId(),a.deptId()))deny();return t;}
