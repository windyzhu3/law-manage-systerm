...
public class TodoCommandService
{
    ...
    @Transactional public TodoInstance autoComplete(Long id,ActionCommand c,Actor a){requireServiceActor(a);if(repeated(id,c))return mapper.selectById(id);TodoInstance t=require(id);validateAction(t,c,"COMPLETE");TodoInstance completed=transition(t,TodoStatus.COMPLETED,null,"COMPLETE_DEFAULT",c,a);for(TodoCompletionHandler h:completionHandlers)if(h.supports(completed))h.complete(completed,c.payload(),a.userId(),a.userName());if(routing!=null)routing.advance(completed,c.payload());return completed;}
    @Transactional public TodoInstance autoReturn(Long id,ActionCommand c,Actor a){requireServiceActor(a);if(repeated(id,c))return mapper.selectById(id);TodoInstance t=require(id);validateAction(t,c,"RETURN");return transition(t,TodoStatus.RETURNED,null,"RETURN_DEFAULT",c,a);}
    @Transactional public TodoInstance autoEscalate(Long id,ActionCommand c,Actor a){requireServiceActor(a);if(repeated(id,c))return mapper.selectById(id);TodoInstance t=require(id);TodoStatus status=TodoStatus.fromCode(t.getStatus());if(status.isTerminal())throw new TodoException("TODO_TERMINAL","Terminal todo cannot be escalated");validateAction(t,c,"ESCALATE");TodoInstance result=transition(t,status,null,"ESCALATE",c,a);mapper.insertSupervisorEscalationNotification(id,java.time.LocalDateTime.now());return result;}
    @Transactional public TodoInstance autoTransfer(Long id,ActionCommand c,Actor a){requireServiceActor(a);if(repeated(id,c))return mapper.selectById(id);TodoInstance t=require(id);TodoStatus status=TodoStatus.fromCode(t.getStatus());if(status.isTerminal())throw new TodoException("TODO_TERMINAL","Terminal todo cannot be transferred");validateAction(t,c,"TRANSFER");Object target=c.payload().get("targetOwnerId");if(target==null)throw new TodoException("TODO_TRANSFER_OWNER_REQUIRED","targetOwnerId is required");return transition(t,status,Long.valueOf(String.valueOf(target)),"TRANSFER",c,a);}
    @Transactional public TodoInstance autoReturnPool(Long id,ActionCommand c,Actor a){requireServiceActor(a);if(repeated(id,c))return mapper.selectById(id);TodoInstance t=require(id);TodoStatus status=TodoStatus.fromCode(t.getStatus());if(status.isTerminal())throw new TodoException("TODO_TERMINAL","Terminal todo cannot return to pool");validateAction(t,c,"RETURN_POOL");if(mapper.returnToPoolConditionally(id,status.code(),a.userName())<=0)throw new TodoException("TODO_CONCURRENT_MODIFICATION","Todo state changed before return to pool");Map<String,Object> log=new HashMap<>();log.put("todoId",id);log.put("actionId",c.actionId());log.put("actionType","RETURN_POOL");log.put("fromStatus",status.code());log.put("toStatus",TodoStatus.CREATED.code());log.put("operatorId",a.userId());log.put("operatorName",a.userName());log.put("opinion",c.opinion());log.put("payloadJson",JSON.toJSONString(c.payload()));if(mapper.insertActionIfAbsent(log)<=0&&mapper.selectActionById(c.actionId())==null)throw new TodoException("TODO_ACTION_LOG_FAILED","Todo action audit failed");t.setStatus(TodoStatus.CREATED.code());t.setOwnerId(null);t.setOwnerDeptId(null);return t;}
