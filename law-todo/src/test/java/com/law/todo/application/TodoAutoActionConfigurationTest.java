...
class TodoAutoActionConfigurationTest
{
    ...
    {
        TodoCommandService commands=mock(TodoCommandService.class);TodoNotificationPort notifications=mock(TodoNotificationPort.class);TodoAutoActionConfiguration config=new TodoAutoActionConfiguration();
        List<TodoAutoActionCapability> values=List.of(config.completeDefaultCapability(commands),config.returnDefaultCapability(commands),config.escalationCapability(commands,notifications),config.transferCapability(commands),config.returnPoolCapability(commands));
    ...
    {
        TodoCommandService commands=mock(TodoCommandService.class);TodoNotificationPort notifications=mock(TodoNotificationPort.class);TodoAutoActionCapability escalation=new TodoAutoActionConfiguration().escalationCapability(commands,notifications);TodoInstance todo=new TodoInstance();todo.setTodoId(4L);todo.setOwnerId(7L);todo.setTitle("Review");
        ...
        assertEquals(List.of("AUTO:4:escalate-DUE:DUE","AUTO:4:escalate-SLA_80:SLA_80","AUTO:4:escalate-SLA_100:SLA_100","AUTO:4:escalate-SLA_150:SLA_150"),captor.getAllValues().stream().map(NotificationCommand::idempotencyKey).toList());
    }
}
