...
The controller creates a `StreamingResponseBody` that closes the artifact in a try-with-resources block and copies `artifact.input()` to the response. Set `Content-Type: application/zip`, `Content-Disposition: attachment; filename="g04-historical-case-preflight.zip"`, `X-Exception-Row-Count`, `X-Exception-CSV-SHA256`, `Cache-Control: no-store`, `X-Content-Type-Options: nosniff`, and `Content-Length` from the artifact. Use an injected `HistoricalMigrationExportAudit` collaborator: call `begin()` to capture request/operator metadata before `exports.export`, record one sanitized failure with unavailable row count if generation rejects, call `generated(rowCount)` only after the artifact exists, then record success only after deferred copy/close completes or one failure in the callback catch path. Never allow metadata/attachment/persistence failure to mask generation or transfer; never log case/body/SQL/exception detail/path/hash. The endpoint annotations are:
...
The final review added five TDD fixes plus a final audit lifecycle correction without changing historical or governance data: the complete eight-key ordered Manifest; JSON/problem+json Blob rejection before save/success state; generation-attempt plus callback-time terminal audit with the correctly encoded title `G-04历史迁移异常清单`; three-attempt same-call cleanup plus `deleteOnExit` fallback; and canonical-root/symlink-swap enforcement. `returnFullResponse` remains opt-in and unchanged for other binary callers.

Fresh disposable MySQL 8.4 verification reached Flyway `0.20.27`. `mvn clean verify` produced 149 Surefire suites and 679 tests: 677 passed, 0 failed, 0 errored, and 2 Windows-only symlink capability tests explicitly skipped. Module totals were `law-file 72`, `law-business 28`, `law-todo 404`, `ruoyi-system 123`, and `ruoyi-admin 52`. On Linux with symlink support, the focused archive-writer suite passed 10/10 with 0 skips. The final backend-only audit correction passed its focused export/controller/adapter matrix 15/15; frontend contract, Schema, UTF-8, Foundation CI contract, production build, focused historical-migration Playwright 4/4, and full Playwright 34/34 remain the preceding verified frontend evidence and were not rerun for that backend-only correction.
